	...
	class FC_Google_Maps_Lite
	{
		...
		function wpgmp_frontend_scripts() {
			...
			$scripts[] = array(
				...
				'deps'   => array( 'flippercode-webfont','wpgmp-jscrollpane','wpgmp-accordion','wpgmp-markercluster','wpgmp-infobox','jquery-masonry','imagesloaded' ),
			);

			if ( $scripts ) {
				foreach ( $scripts as $script ) {
					wp_register_script( $script['handle'], $script['src'], $script['deps'], false, $where );
				}
			}
	...
	<?php
		...
		function wpgmp_show_sample_admin_notice__info(){
			...
			if (isset($_GET['page']) && !empty($_GET['page']) && strpos(sanitize_key($_GET['page']), 'wpgmp') !== false) {
				...
				if(isset($ignore['wpgmp_hide_sample_notice']) && !empty($ignore['wpgmp_hide_sample_notice']) && $ignore['wpgmp_hide_sample_notice'] == 'true')
				return;

				if(isset($ignore['wpgmp_show_sample_notice_time']) && !empty($ignore['wpgmp_show_sample_notice_time'])){
