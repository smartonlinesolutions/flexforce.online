...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="mediz-single-article mediz-blog-quote-format" >
	<?php 
		// post title
		...
		if( !empty($thumbnail_id) ){
			...
			echo '<div class="mediz-blog-quote-background" ' . gdlr_core_esc_style(array(
				'background-image' => $quote_background
			)) . ' ></div>';
		}

		echo '<div class="mediz-blog-quote gdlr-core-quote-font" >&#8220;</div>';
		echo '<div class="mediz-blog-content-wrap" >';
		if( !empty($blockquote) ){
			echo '<div class="mediz-blog-quote-content gdlr-core-info-font">' . gdlr_core_content_filter($blockquote, true) . '</div>';
		}
		if( !empty($author) ){
			echo '<div class="mediz-blog-quote-author gdlr-core-info-font" >' . gdlr_core_escape_content(gdlr_core_text_filter($author)) . '</div>';
		}
		echo '</div>';
		echo '</div>';
	?>
