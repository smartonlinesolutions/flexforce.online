<?php
	...
	if( mediz_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				...
				'<span class="mediz-text mediz-title-font" >' . esc_html__( 'Previous Post', 'mediz' ) . '</span>' .
				'%link' . 
				'</span>',
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link( 
				...
				'<span class="mediz-text mediz-title-font" >' . esc_html__( 'Next Post', 'mediz' ) . '</span>' . 
				'%link' . 
				'</span>',
				'<span class="mediz-single-nav-title mediz-title-font" >%title</span>'
			);
	...
	if( mediz_get_option('general', 'blog-social-share', 'enable') == 'enable' ){
		if( class_exists('gdlr_core_pb_element_social_share') ){
			...
			echo '<div class="mediz-single-social-share mediz-item-rvpdlr clearfix" >';
	...
	// tag 
	echo '<div class="mediz-single-magazine-author-tags clearfix" >';
	...
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		...
		$related_post_args = array(
			...
			'blog-column-style' => mediz_get_option('general', 'related-post-blog-column-style', 'style-1'),
		...
		if( $query->have_posts() ){
			...
			echo '<div class="mediz-single-related-post-wrap mediz-item-rvpdlr" >';
			echo '<h3 class="mediz-single-related-post-title mediz-item-mglr" >' . esc_html__('Related Posts', 'mediz') . '</h3>';
