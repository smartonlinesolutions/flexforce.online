<?php
	...
	echo '<div class="mediz-single-author-tags clearfix" >';
	...
	if( mediz_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				'<span class="mediz-single-nav-area-left" >%link</span>',
				'<span class="mediz-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($prev_post->ID))) . ' ></span>' . 
				'<span class="mediz-single-nav-area-overlay" ></span>' . 
				'<span class="mediz-single-nav mediz-single-nav-left">' . 
				'<i class="arrow_left" ></i>' . 
				'<span class="mediz-text" >' . esc_html__( 'Previous Post', 'mediz' ) . '</span>' .
				'<span class="mediz-single-nav-title mediz-title-font" >%title</span>' .
				'</span>'
			);
		}
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link(
				'<span class="mediz-single-nav-area-right" >%link</span>',
				'<span class="mediz-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($next_post->ID))) . ' ></span>' .
				'<span class="mediz-single-nav-area-overlay" ></span>' .
				'<span class="mediz-single-nav mediz-single-nav-right">' . 
				'<span class="mediz-text" >' . esc_html__( 'Next Post', 'mediz' ) . '</span>' . 
		...
		if( !empty($prev_post_html) || !empty($next_post_html) ){
			echo '<div class="mediz-single-nav-area clearfix" >' . $prev_post_html . $next_post_html . '</div>';
		}
	}
	...
	if( comments_open() || get_comments_number() ){
		echo '<div class="mediz-comment-wrapper">';
		echo '<div class="mediz-comment-container mediz-container">';
		echo '<div class="mediz-comment-content mediz-item-pdlr">';
		comments_template();
		echo '</div>';
		echo '</div>';
		echo '</div>';
