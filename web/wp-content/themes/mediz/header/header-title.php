<?php
	...
	$title_style = mediz_get_option('general', 'default-title-style', 'small');
	$title_align = mediz_get_option('general', 'default-title-align', 'left');

	// header gradient
	$header_gradient = mediz_get_option('general', 'page-title-top-bottom-gradient', 'none');

	// for index page
	if( is_home() ){
		$main_head = true;
		$page_title = esc_html__('Blog', 'mediz');
	...
	// for pages
		...
		if( empty($post_option['enable-breadcrumbs']) || $post_option['enable-breadcrumbs'] == 'default' ){
			$breadcrumbs = mediz_get_option('general', 'enable-breadcrumbs', 'disable');
		}else{
	...
	// search page
		...
		if( have_posts() ){
			$page_title = esc_html__('Search Results', 'mediz');
	...
	// archive page
		...
		if( !class_exists('gdlr_core_pb_element_product') && 
			...
			$page_title = esc_html__('Category', 'mediz');
			...
			$page_title = esc_html__('Tag', 'mediz');
	...
	if( (empty($post_option['enable-page-title']) || $post_option['enable-page-title'] == 'enable') && (!empty($page_title) || !empty($page_caption)) ){	
		...
		$page_title = apply_filters('mediz_page_title', $page_title);
		$page_caption = apply_filters('mediz_page_caption', $page_caption);

		$extra_class  = ' mediz-style-' . $title_style;
		$extra_class .= ' mediz-' . $title_align . '-align';

		echo '<div class="mediz-page-title-wrap ' . esc_attr($extra_class) . '" ' . gdlr_core_esc_style(array(
			'background-image' => empty($title_background)? '': $title_background
		)) . '>';
		echo '<div class="mediz-header-transparent-substitute" ></div>';
		echo '<div class="mediz-page-title-overlay" ' . gdlr_core_esc_style(array(
		...
		if( $header_gradient == 'top' || $header_gradient == 'both' ){
			echo '<div class="mediz-page-title-top-gradient" ></div>';
		}
		if( $header_gradient == 'bottom' || $header_gradient == 'both' ){
			echo '<div class="mediz-page-title-bottom-gradient" ></div>';
		}
		echo '<div class="mediz-page-title-container mediz-container" >';
		echo '<div class="mediz-page-title-content mediz-item-pdlr" ' . gdlr_core_esc_style(array(
		...
		if( !empty($page_title) ){
			if( !empty($main_head) ){
				echo '<h1 class="mediz-page-title" ' . gdlr_core_esc_style(array(
				...
				echo '<h3 class="mediz-page-title" ' . gdlr_core_esc_style(array(
					'font-size' => empty($title_size['title-size'])? '': $title_size['title-size'],
					'font-weight' => empty($title_weight['title-weight'])? '': $title_weight['title-weight'],
					'text-transform' => (empty($title_transform) || $title_transform == 'default')? '': $title_transform,
					'letter-spacing' => empty($title_size['title-letter-spacing'])? '': $title_size['title-letter-spacing'],
		...
		if( !empty($page_caption) ){
			echo '<div class="mediz-page-caption" ' . gdlr_core_esc_style(array(
		...
		if( function_exists('bcn_display') && $breadcrumbs == 'enable' ){
			...
			echo '<div class="mediz-breadcrumbs" ' . gdlr_core_esc_style(array(
