<?php 
	...
	*/	
	...
	if( !function_exists('mediz_get_style_custom') ){
		function mediz_get_style_custom($local = false){

			$upload_dir = wp_upload_dir();
			$filename = '/mediz-style-custom.css';
	...
	add_action( 'wp_enqueue_scripts', 'mediz_include_scripts', 11 );
	if( !function_exists('mediz_include_scripts') ){
		function mediz_include_scripts(){
			...
			// include site style
			wp_enqueue_style('mediz-style-core', get_template_directory_uri() . '/css/style-core.css');
			if( is_rtl() ){
				wp_enqueue_style('mediz-frontend-rtl', get_template_directory_uri() . '/css/frontend-rtl.css');
			}
			
			if( !is_customize_preview() ){
				wp_enqueue_style('mediz-custom-style', mediz_get_style_custom());
			}
			...
			wp_enqueue_script('mediz-script-core', get_template_directory_uri() . '/js/script-core.js', array('jquery', 'jquery-effects-core'), '1.0.0', true );
	...
	add_action( 'wp_enqueue_scripts', 'mediz_include_child_scripts', 999 );
	if( !function_exists('mediz_include_child_scripts') ){
		function mediz_include_child_scripts(){
			if( is_child_theme() ){
				wp_enqueue_style('mediz-child-theme-style', get_stylesheet_uri());
			}
		}
	}