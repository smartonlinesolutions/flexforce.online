...
<?php 
	...
	if( $header_style == 'side' ){
		...
		$header_side_class  = ' mediz-style-side';
		$header_side_style = mediz_get_option('general', 'header-side-style', 'top-left');

		switch( $header_side_style ){
			case 'top-left': 
				$header_side_class .= ' mediz-style-left';
				$body_wrapper_class .= ' mediz-left';
				break;
			case 'top-right': 
				$header_side_class .= ' mediz-style-right';
				$body_wrapper_class .= ' mediz-right';
				break;
			case 'middle-left':
			case 'middle-left-2':
				$header_side_class .= ' mediz-style-left mediz-style-middle';
				$body_wrapper_class .= ' mediz-left';
				break;
			case 'middle-right': 
			case 'middle-right-2': 
				$header_side_class .= ' mediz-style-right mediz-style-middle';
				$body_wrapper_class .= ' mediz-right';
				break;
			default: 
				break;
		...
		$header_side_style = mediz_get_option('general', 'header-side-toggle-style', 'left');

		$header_side_class  = ' mediz-style-side-toggle';
		$header_side_class .= ' mediz-style-' . $header_side_style;
		$body_wrapper_class .= ' mediz-' . $header_side_style;

	}else if( $header_style == 'boxed' ){

		$body_wrapper_class .= ' mediz-with-transparent-header';
		...
		if( $header_background_style == 'transparent' ){
			if( $header_style == 'plain' ){
				...
				$body_wrapper_class .= ' mediz-with-transparent-navigation';
			}
		}
	}
	...
	if( $layout == 'full' && in_array($header_style, array('plain', 'bar', 'boxed')) ){
		$body_wrapper_class .= ' mediz-with-frame';
	}
	...
	if( empty($post_option['enable-header-area']) || $post_option['enable-header-area'] == 'enable' ){
		...
		$body_outer_wrapper_class = ' mediz-header-disable';
	}
	
	// preload
	$preload = mediz_get_option('plugin', 'enable-preload', 'disable');
	if( $preload == 'enable' ){
		echo '<div class="mediz-page-preload gdlr-core-page-preload gdlr-core-js" id="mediz-page-preload" data-animation-time="500" ></div>';
	}
?>
<div class="mediz-body-outer-wrapper <?php echo esc_attr($body_outer_wrapper_class); ?>">
	<?php
		...
		if( $layout == 'boxed' ){
			if( !empty($post_option['body-background-type']) && $post_option['body-background-type'] == 'image' ){
				echo '<div class="mediz-body-background" ' . gdlr_core_esc_style(array(
				...
				$background_type = mediz_get_option('general', 'background-type', 'color');
				if( $background_type == 'image' ){
					echo '<div class="mediz-body-background" ></div>';
				}
			}
		}
	?>
	<div class="mediz-body-wrapper clearfix <?php echo esc_attr($body_wrapper_class); ?>">
	<?php  
		...
		if( empty($post_option['enable-header-area']) || $post_option['enable-header-area'] == 'enable' ){
			...
			if( $header_style == 'side' || $header_style == 'side-toggle' ){
				...
				echo '<div class="mediz-header-side-nav mediz-header-background ' . esc_attr($header_side_class) . '" id="mediz-header-side-nav" >';

				// header - logo area
				get_template_part('header/header-style', $header_style); 

				echo '</div>';
				echo '<div class="mediz-header-side-content ' . esc_attr($header_side_class) . '" >';
				...
				if( $header_style == 'plain' ){
					if( $header_background_style == 'transparent' ){
						$close_div = true;
						echo '<div class="mediz-header-background-transparent" >';
					}
				}else if( $header_style == 'boxed' ){
					$close_div = true;
					echo '<div class="mediz-header-boxed-wrap" >';
				}
